...
die() {
    eerror "$*"
    read -t 5 -p "Press Enter to open a console or wait 5s to reboot"
    if [ $? -eq 0 ]; then
...
attach_nbd_device() {
    ...
    [ "$export_uri" = "" ] && die "Unable to load metadatas..."

    local nbd_host=$(echo $export_uri | sed -n 's#nbd://\(.*\):.*$#\1#p')
    local nbd_port=$(echo $export_uri | sed -n 's#nbd://.*:\(.*\)$#\1#p')
    [ "$nbd_host" = "" -o "$nbd_port" = "" ] && die "Parse error of $export_uri"


    # Checking if device is already attached
    run @xnbd-client -c /dev/nbd${device} >/dev/null 2>/dev/null
    if [ $? -eq 0 ]; then
        return
    fi
    ...
    [ $? -ne 0 ] && die "Device connection failed."
    ...
    # Refreshing the nbd's partitions
    partx -u /dev/nbd${device}
...
# Return all unique values for $what from one or multiple files
get_unique() {
    what=$1
    shift
    sed -ne "s/$what/\1/p" $@ | sort | uniq
}
